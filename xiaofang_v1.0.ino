...
//焊接点阵之前，下面这行前面的两条斜杠要删掉，确认点阵安装没问题了，在把两条斜杠加上
//#define DEBUG
//串口调试开关
//#define Serial_DEBUG
...
void eyes_dai_draw(byte eyes_x,byte eyes_y){
  //画左眼
  lc.setLed(eyes_x,eyes_y,1);
  lc.setLed(eyes_x,eyes_y+1,1);
  lc.setLed(eyes_x+1,eyes_y,1);
  lc.setLed(eyes_x+1,eyes_y+1,1);
  //画右眼
  lc.setLed(eyes_x+4,eyes_y,1);
  lc.setLed(eyes_x+4,eyes_y+1,1);
  lc.setLed(eyes_x+5,eyes_y,1);
  lc.setLed(eyes_x+5,eyes_y+1,1);
}
...
/*
//迷宫地图写入函数
void map_w(byte page){//（页地址0-7）把该地图数据写入到指定页的0-127空间

  byte map_data[128]={//地图0
  0b10000000,0b00000111,0b11111111,0b11111111,
  0b10000000,0b00000100,0b11110000,0b00000001,
  0b10011111,0b11111100,0b11110000,0b00000001,
  0b10000000,0b00000000,0b00000011,0b11110001,
  ...
  0b11111111,0b11111111,0b11000001,0b11111111,
    ...
    for(int i=0;i<128;i++){
         myflash.iic_write_byte(page,map_data[i]);    
    }
}
*/
      ...
      if(kx>0.5){
        ...
        for(int i=0;i<100;i++){
          if(imu.ax()<-0.5){
            lc.clearDisplay();  
            bagua_beeper();
            lc.bitmap(shaizi_all[random(0,6)]);
            lc.UpLoad();
            i=50;
          }
        }

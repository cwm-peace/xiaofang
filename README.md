# xiaofang
<p>小方1.0-巴掌宽护胸毛<font color='red'>供个人爱好者学习研究，请勿商用</font></p>
...
1. 打印PCB和外壳
     ...
     1. 下载安装佳立创EDA（专业版），然后打开软件，在首页中间选择“导入专业版”。
     2. 然后找到开源文件里的<mark>小方开源v1.0(硬件设计佳立创EDA专业版).epro</mark>这个文件。
     3. 成功打开后，然后打开PCB文件，就可以直接在软件里下单打印PCB，PCB选1.6mm厚度，4层板。
     4. 注册账户每个月可以领免费卷，白嫖。外壳文件也可以用同样的方式在软件里下单，我选的材料是<b>9000R，白色</b>。

2. 根据物料清单到淘宝采购零件，型号尺寸别买错了

     1. 打开佳立创EDA（专业版），打开PCB原理图，对照着零件位置焊接所有零件，但<b>注意</b>点阵先不要焊接。除了点阵都焊接好之后，根据教程烧录bootloader，需要用到另外一块可以正常编程的arduino uno板子，具体看开源文件里的教程。
     2. bootloader烧录好之后，就可以直接用type-c数据线直接给小方编程了，打开arduinoIDE打开主工程<mark>xiaofang_v1.0.ino</mark>，然后把对应的库文件放到指定位置（后面会说），然后按照代码里的注释说明按顺序编译上传。

3. 验证点阵引脚顺序

     1. 根据arduino里的代码提示，先执行点阵测试，编译上传
     2. 然后在电路板通电的状态下，把点阵屏插进去，尽量让每个引脚都挨上焊盘，看是不是每个灯都能亮，如果接反了，无论你怎么调整按压姿势，都无法让每一个灯都亮（不用同时亮）。
     3. 测试确定每个灯都能亮之后，就可以把点阵焊接上了，然后把多余的引脚剪掉，硬件就差不多搞定了，然后就可以把电池接上，外壳装上就可以开心的编程玩耍了。
     4. 如果硬件有问题，检查焊接，和芯片方向是否放对了。

4. 代码编译顺序

     1. 请按照代码注释说明的顺序操作，先把地图数据写入到外部的flash里，不然迷宫游戏过了第一关就会卡死，因为迷宫过关后会从flash读取新的地图数据，如果flash里没有地图数据，迷宫游戏就无法正常运行。
     2. 主要工程文件是xiaofang_v1.0.ino，是ArduinoIDE里写的，直接用Arduino打开即可。
     3. 然后需要把这几个文件夹放到Arduino的libraries文件夹里，这几个文件夹都是使用到的相关库文件（basicMPU6050、iic_16k、ip5306ck、LedControl、LowPower、she）。

     | 库文件名     | 描述                                 |
     | ------------ | ------------------------------------ |
     | basicMPU6050 | 主要用于驱动MUP6050                  |
     | iic_16k      | 用于驱动AT24C6存储芯片               |
     | ip5306ck     | 用于读取电池电量信息                 |
     | LedControl   | 用于驱动点阵                         |
     | LowPower     | 用于开启ATMEGA328P低功耗             |
     | she          | 主要是存放的贪吃蛇的定义以及部分功能 |

​	以上几个库一个都不能少。ATMEGA328P的引导程序烧录教程在“如何用Arduino给ATMEGA328p裸片烧录引导程序”这个文件夹里，跟着操作就搞定了。

## 声明

> 我就是业余爱好，代码水平是真不行，我已经尽我所能写了备注，但遗憾的是有时候依然连我自己都读不懂，各位自求多福吧。有些参数没搞明白之前最好不要改，不然会出些莫名其妙的问题。现在的代码刚好能跑，目前没有太大的bug，相对比较流畅。我也在抽空不断优化，如果你是代码老司机可以帮忙优化优化。

#### 代码的大概原理

所有功能都放在一个大循环里，每个功能有个标志位变量，当这个变量为1时表明当前选择的是这个功能，其他功能的标志位会被置零，所以在大循环里反复执行这个功能。

然后在每个功能里设置了手势检测，如果读取到进入菜单的操作（向后甩）就会立即进入菜单切换界面，通过左右倾斜选择功能，向上倾斜就确定进入该功能。这个菜单选择操作就会设置功能的标志位变量，让其中一个为1，其他全为零。（除了温度、代码版本、声音开关这几个不用，这几个进入后，会自动回到进入菜单时的功能里。）

#### 各功能描述
插上usb就会进入充电界面，会一直显示充电动画，直到充满，这个时候无法使用任何功能。只有拔下来才能进入后面的功能，每次插电会自动重启主控，拔下来后会开始自动校正6050的偏差，这个大概会等待10秒左右。但后续休眠被唤醒就很快。
...
1. 点阵驱动里有个函数，它的名字叫setIntensity(int intensity)，参数范围0-15，但<font color='red'>千万千万</font>不要把参数设置到<kbd>4</kbd>以上太长时间，不然很快就会冒烟。点阵全功率电流太大，电路设计没法承载。
2. 电路板放主控芯片那面的所有零件都可以用加热盘一次性焊接好，另外一面有一个点阵驱动芯片和几个电容电阻需要手动焊上去。
   ...
   因为这里你一旦放错了方向，点阵不会正常工作，然后你还拆不下来，等于干废一块板子。别问我为什么知道，实践出真知
3. 测试点阵方向ok之后，就可以恢复“#define DEBUG”前面的两条杠“//“，再重新上传完整代码进去，然后焊接上电池，万事大吉！！！！
